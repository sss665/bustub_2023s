...
class LockManager {
 ...
 private:
  ...
  auto AreLocksUpgrade(LockMode l1, LockMode l2) -> bool;
  auto CanGetLock(std::shared_ptr<LockRequestQueue> &lock_request_queue, Transaction *txn, LockMode lock_mode) -> bool;
  auto UpgradeLockTable(std::shared_ptr<LockRequestQueue> &lock_request_queue, Transaction *txn, LockMode lock_mode,
                        const table_oid_t &oid) -> bool;
  auto GetTxnSet(Transaction *txn, LockMode lock_mode) -> std::shared_ptr<std::unordered_set<table_oid_t>>;
  auto GetTxnRowSet(Transaction *txn, LockMode lock_mode)
      -> std::shared_ptr<std::unordered_map<table_oid_t, std::unordered_set<RID>>>;
  auto UpgradeLockRow(std::shared_ptr<LockRequestQueue> &lock_request_queue, Transaction *txn, LockMode lock_mode,
                      const table_oid_t &oid, const RID &rid) -> bool;
  auto AreLocksCompatible(LockMode l1, LockMode l2) -> bool;
  auto IsolationCheck(Transaction *txn, LockMode lock_mode) -> void;
  ...
  auto CheckAppropriateLockOnTable(Transaction *txn, const table_oid_t &oid, LockMode row_lock_mode) -> void;
  auto FindCycle(txn_id_t source_txn, std::vector<txn_id_t> &path, std::unordered_set<txn_id_t> &visited,
                 txn_id_t *abort_txn_id) -> bool;
  void UnlockAll();
  auto PrintLockMode(LockMode lockmode) -> void;
  ...
  std::unordered_map<txn_id_t, std::vector<table_oid_t>> waits_table_;
  std::mutex waits_table_latch_;
  std::unordered_map<txn_id_t, std::vector<RID>> waits_rid_;
  std::mutex waits_rid_latch_;
};
