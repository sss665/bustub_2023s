...
auto NestedLoopJoinExecutor::Next(Tuple *tuple, RID *rid) -> bool {
	...
	for(;status1_;status1_ = left_executor_->Next(&left_tuple_,&left_rid_)){
		for(bool status2 = right_executor_->Next(&right_tuple,&right_rid);status2;status2 = right_executor_->Next(&right_tuple,&right_rid)){
			auto value = filter_expr->EvaluateJoin(&left_tuple_, left_executor_->GetOutputSchema(),&right_tuple,right_executor_->GetOutputSchema());
		...
		if(plan_->GetJoinType() == JoinType::LEFT && !done_){
			...
			for (Column col: right_executor_->GetOutputSchema().GetColumns()) {
